...
Public Class tambahdatapemilih
#Region "geser"
    Sub New()
...
#End Region
    ...
    Private Sub btn1_click(sender As Object, e As EventArgs) Handles Button1.Click
        If Not CustomHeaderTextboxMD4.DataError Then
            buka()
            Try
                cmd = New SqlCommand("INSERT INTO datapemilih(username,password,status) VALUES('" & CustomHeaderTextboxMD4.TextBoxText & "', '" & CustomHeaderTextboxMD2.TextBoxText & "','0')", cnn)
                cmd.ExecuteNonQuery()
                MsgBox("Data Sukses Disimpan", MsgBoxStyle.Information)
                hapus()
                Form1.Datapemilih21.isi()
    ...
    Private Sub CustomHeaderTextboxMD4_LostFocus(sender As Object, e As EventArgs) Handles CustomHeaderTextboxMD4.LostFocus
        buka()
        Try
            cmd = New SqlCommand("SELECT username FROM datapemilih WHERE username='" & CustomHeaderTextboxMD4.TextBoxText & "'", cnn)
